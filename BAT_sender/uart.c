...
/*
 * Initialize the UART to 115200 Bd, tx/rx, 8N1.
 */
void
uart_init(void)
{
	UCSR0A = _BV(U2X0);             /* improve baud rate error by using 2x clk */
	UBRR0L = 16;	//1152400 Baud @ 16 MHz
...
int uart_getchar(FILE *stream) {
	...
	/* behaviour similar to Unix stty ICRNL */
			...
			if ((c >= (uint8_t)' ' && c <= (uint8_t)'\x7e') || c >= (uint8_t)'\xa0'){
				if (cp == b + RX_BUFSIZE - 1) uart_putchar('\a', stream);
			else{
				*cp++ = c;
				uart_putchar(c, stream);
			}
		...
		switch (c){
			...
			case 'r' & 0x1f:
				uart_putchar('\r', stream);
				for (cp2 = b; cp2 < cp; cp2++) uart_putchar(*cp2, stream);
				break;
